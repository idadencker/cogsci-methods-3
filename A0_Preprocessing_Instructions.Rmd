...
```{r}
#Give variable of participant ID the same name
demo <- demo %>%
  rename(ID=Child.ID)
...
merge_first_time <- merge(demo_clean,LU_clean, all=T)

merge_second_time <- merge(merge_first_time, token_clean, all=T)
...
```{r}
new_data_set <- merge_second_time%>%
  filter(Visit=="1")%>%
  select(ADOS, MullenRaw, ExpressiveLangRaw, Socialization)%>%
...
```{r}
merge_final <- merge(new_data_set,merge_second_time, all=T)
```
...
write.csv(merge_final,"/Users/idahelenedencker/Desktop/CognitiveScience/3. semester/Methods 3/Git/cogsci-methods-3\\mergeddata.csv", row.names = FALSE)
