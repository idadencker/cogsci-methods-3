...
In other words, we need to define a few parameters:
- average MLU for ASD (population mean) at Visit 1 (1.5) and average individual deviation from that (population standard deviation) (0.5)
- average MLU for TD (population mean) at Visit 1 (1.5) and average individual deviation from that (population standard deviation) (0.3)
- average change in MLU by visit for ASD (population mean) (0.4) and average individual deviation from that (population standard deviation) (0.4)
- average change in MLU by visit for TD (population mean) (0.6) and average individual deviation from that (population standard deviation) (0.2)
- an error term. Errors could be due to measurement, sampling, all sorts of noise. 0.2
...
B0A <- rnorm(n, mean = mu_ASD, sd = sd_ASD)
B0TD <- rnorm(n, mean = mu_TD, sd = sd_TD)
B1A <- rnorm(n, mean = mu_change_ASD, sd = sd_change_ASD)
B1TD <- rnorm(n, mean = mu_change_TD, sd = sd_change_TD)
EA <- rnorm(n, mean = 0, sd = e)
ETD <- rnorm(n, mean = 0, sd = e)
...
```{r}
#Fit a model with the simulated data
...
```{r}
#try different sample sizes/power analysis
```
